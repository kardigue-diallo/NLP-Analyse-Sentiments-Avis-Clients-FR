...
# Liste des mots vides (Stopwords FR) optimisée
stop_words_fr = set([
    # Articles et déterminants
    'le', 'la', 'les', 'de', 'du', 'des', 'un', 'une', 'ce', 'cette', 'ces', 'mon', 'ton', 'son', 'notre', 'votre', 'leur',
    # Prépositions et conjonctions (Attention à garder 'pas' et les négations hors d'ici !)
    'et', 'en', 'que', 'qui', 'dans', 'pour', 'sur', 'au', 'aux', 'par', 'avec', 'sans', 'sous', 'ou', 'donc', 'or', 'ni', 'car', 'mais',
    # Pronoms
    'je', 'tu', 'il', 'elle', 'on', 'nous', 'vous', 'ils', 'elles', 'se', 'me', 'te', 'lui', 'leur', 'y', 'en',
    # Verbes être / avoir fréquents (souvent neutres en sentiment)
    'suis', 'es', 'est', 'sommes', 'êtes', 'sont', 'été', 'ai', 'as', 'a', 'avons', 'avez', 'ont', 'eu'
])
...
if mode == "📝 Analyse d'un Avis Unique":
    ...
    if btn_analyser or avis:
        ...
        # 🔍 Ligne de débogage pour afficher tous les mots détectés dans la liste
        st.write("🔍 [DEBUG] Détails bruts des mots :", details_mots)
...
# ==========================================
    ...
    # 1. Option pour choisir entre le fichier par défaut ou un fichier personnel
    type_source = st.radio(
        "Source des données :", 
        ["Utiliser le fichier d'exemple par défaut", "Téléverser mon propre fichier CSV"]
    )
    
    df = None
    
    if type_source == "Utiliser le fichier d'exemple par défaut":
        # Création du DataFrame d'exemple intégré
        data_exemple = {
            "texte": [
                "Le service était rapide et le livreur très sympathique, je recommande !",
                "Absolument horrible, la commande est arrivée avec une heure de retard et c'était froid.",
                "Correct sans plus, rien d'exceptionnel mais ça fait le travail.",
                "Une catastrophe, le plat était immangeable et le service client injoignable.",
                "Franchement bluffé par la qualité des produits, c'était chaud et délicieux.",
                "Le pire repas de ma vie, plus jamais je ne commanderai chez vous.",
                "Service client au top, ils ont su régler mon problème de livraison très rapidement.",
                "Pas mauvais, mais le rapport qualité-prix n'y est clairement pas.",
                "Une expérience incroyable du début à la fin, bravo à toute l'équipe !",
                "Mediocre, la portion est minuscule pour un prix exorbitant.",
                "J'ai adoré le packaging et la fraîcheur, un vrai régal.",
                "L'application plante sans arrêt et impossible de joindre le support, fuyez !"
            ]
        }
        df = pd.DataFrame(data_exemple)
    ...
    # 2. Traitement si le DataFrame est disponible (via l'exemple ou l'upload)
    if df is not None:
